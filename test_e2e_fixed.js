...
async function runTests() {
  ...
  if (createManager.status === 201 && createManager.body.success) {
    ...
    console.log('   Manager temp password:', createManager.body.temporaryPassword);
  ...
  if (createSalesperson.status === 201 && createSalesperson.body.success) {
    ...
    console.log('   Salesperson temp password:', createSalesperson.body.temporaryPassword);
  ...
  console.log('   Manager login status:', mgrLogin.status, 'success:', mgrLogin.body.success);
  console.log('   Manager login body keys:', Object.keys(mgrLogin.body));
  console.log('   Manager login body:', JSON.stringify(mgrLogin.body));
  if (mgrLogin.status === 200 && mgrLogin.body.success) {
    managerToken = mgrLogin.body.token;
    console.log('   Manager token length:', managerToken.length);
  ...
  console.log('   Salesperson login status:', spLogin.status, 'success:', spLogin.body.success);
  if (spLogin.status === 200 && spLogin.body.success) {
    salespersonToken = spLogin.body.token;
    console.log('   Salesperson token length:', salespersonToken.length);
  ...
  if (pendingCustomer) {
    ...
    assert('Customer profile ID', false, { count: getCustomers.body.customers?.length, userId: testCustomerUserId });
  }
  ...
  assert('Admin product created', adminProductResult.status === 201 && adminProductResult.body.success, adminProductResult.body);
  if (adminProductResult.body.product?.id) testProductId = adminProductResult.body.product.id;
  ...
  console.log('   Manager token exists:', !!managerToken, 'length:', managerToken?.length);
